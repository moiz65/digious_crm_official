    ...
    return attendanceData.filter((record) => {
      ...
      switch (selectedDate) {
        ...
        case "custom":
          ...
          return (
            recordDate.getTime() >= start.getTime() &&
            recordDate.getTime() <= end.getTime()
          );
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-slate-600 font-medium">
                📅 {getDateDisplay()} • {dateFilteredData.length} of{" "}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <AttendanceOverviewCard
                stats={stats}
                totalEmployees={dateFilteredData.length}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-xl">
                    ...
                    <span className="text-2xl font-bold text-emerald-600">
                      {dateFilteredData.length > 0
                        ? Math.round(
                            (stats.present / dateFilteredData.length) * 100,
                          )
                        : 0}
                      %
                    </span>
                  ...
                  <div className="flex items-center justify-between p-4 bg-amber-50 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-semibold text-slate-800">
                        Avg Breaks per Employee
                      </span>
                    </div>
                    <span className="text-2xl font-bold text-amber-600">
                      {dateFilteredData.length > 0
                        ? (stats.totalBreaks / dateFilteredData.length).toFixed(
                            1,
                          )
                        : 0}
                    </span>
            ...
            <tbody className="divide-y divide-slate-200">
              {paginatedData.length === 0 ? (
                <tr>
                  <td
                    colSpan="7"
                    className="px-6 py-12 text-center text-slate-500 w-100"
                  >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {record.late_by_minutes > 0 && (
                          <span className="text-xs text-amber-600 font-medium">
                            {formatMinutesShort(record.late_by_minutes)}
                          </span>
                        )}
                      </div>
                    </td>
      ...
      {data.length > itemsPerPage && (
        <div className="flex items-center justify-between p-4 bg-white rounded-2xl shadow-lg border border-slate-200">
          <div className="text-sm text-slate-600">
            ...
            of <span className="font-semibold">{data.length}</span> records
          </div>
        ...
        <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-400 mt-1">
                    {employee.attendance_date
                      ? new Date(employee.attendance_date).toLocaleDateString(
                          "en-CA",
                        )
                      : ""}
                  </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-3">
            ...
            <div className="p-4 bg-white rounded-xl border border-slate-100 shadow-sm">
              ...
              <div className="text-xs text-slate-400 mt-1">
                {formatMinutesShort(employee.total_break_duration_minutes || 0)}{" "}
                total
              </div>
            </div>
            <div className="p-4 bg-white rounded-xl border border-slate-100 shadow-sm">
                ...
                {(() => {
                  ...
                  if (employee.check_out_time) {
                    return formatMinutesShort(
                      employee.net_working_time_minutes || 0,
                    );
                  }
                  ...
                  if (employee.check_in_time && employee.attendance_date) {
                    try {
                      ...
                      const checkInTimeInMinutes = parseTimeToMinutes(
                        employee.check_in_time,
                      );

                      // Calculate difference (handling day boundary)
                      let diffMins =
                        currentTimeInMinutes - checkInTimeInMinutes;
                      ...
                      const breakMins =
                        employee.total_break_duration_minutes || 0;
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-slate-500">
                          {breakItem.reason || "No reason"}
                        </div>
                      </div>
                    </div>
                    <div className="text-sm text-slate-700 font-semibold">
                      {formatMinutesShort(
                        breakItem.break_duration_minutes || 0,
                      )}
                    </div>
                  </div>
                ))}
