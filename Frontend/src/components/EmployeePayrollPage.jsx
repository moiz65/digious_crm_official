    ...
    >
      ...
      >
        ...
        <div className="p-6 space-y-5">
              ...
              <div className="p-4 space-y-2">
                ...
                {payroll.late_days > 0 && !payroll.late_deduction && (
                  <div className="flex justify-between text-sm text-slate-400">
                    <span>
                      {payroll.late_days} late — no deduction (need 2+)
                    </span>
                    <span>0</span>
                  </div>
                )}
          ...
          {/* Leave Balance */}
          {(payroll.casual_leaves_total > 0 ||
            payroll.sick_leaves_total > 0 ||
            payroll.annual_leaves_total > 0) && (
            <div className="border border-violet-200 rounded-xl overflow-hidden">
              ...
              <div className="p-4 grid grid-cols-3 gap-3">
                {[
                  {
                    label: "Casual",
                    used: payroll.casual_leaves_used || 0,
                    total: payroll.casual_leaves_total || 0,
                    color: "violet",
                  },
                  {
                    label: "Sick",
                    used: payroll.sick_leaves_used || 0,
                    total: payroll.sick_leaves_total || 0,
                    color: "blue",
                  },
                  {
                    label: "Annual",
                    used: payroll.annual_leaves_used || 0,
                    total: payroll.annual_leaves_total || 0,
                    color: "teal",
                  },
                ].map((l) => {
                  const pct = l.total
                    ? Math.min(100, (l.used / l.total) * 100)
                    : 0;
                  return (
                    <div
                      key={l.label}
                      className="text-center p-3 bg-white border border-slate-100 rounded-lg"
                    >
                      ...
                      <div
                        className={`mt-2 bg-${l.color}-100 rounded-full h-1.5 overflow-hidden`}
                      >
                        <div
                          className={`bg-${l.color}-500 h-1.5 rounded-full`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
          ...
          <div className="flex items-start gap-3 p-4 bg-amber-50 rounded-xl border border-amber-200">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 mt-1">
                Every 3 late arrivals = 1 day salary deduction (Daily Rate =
                Base Salary ÷ 30).
                {payroll.late_days > 0 && (
                  <span className="block mt-1 font-medium">
                    This month: {payroll.late_days} late →{" "}
                    {payroll.late_deduction_days ||
                      Math.floor(payroll.late_days / 3)}{" "}
                    day(s) deducted = PKR {formatNum(payroll.late_deduction)}
                  </span>
                )}
              </p>
            </div>
  ...
  return (
    ...
    >
      <div className="p-4 md:p-6 lg:p-8 max-w-full mx-auto space-y-6">
        ...
        {/* ═══ Leave Balance ═══ */}
        {latest &&
          (latest.casual_leaves_total > 0 ||
            latest.sick_leaves_total > 0 ||
            latest.annual_leaves_total > 0) && (
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-2xl p-5 shadow-sm">
              <h3 className="text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2">
                <Calendar className="h-4 w-4 text-violet-600" /> Annual Leave
                Balance
              </h3>
              <div className="grid grid-cols-3 gap-4">
                {[
                  {
                    label: "Casual",
                    used: latest.casual_leaves_used || 0,
                    total: latest.casual_leaves_total || 0,
                    colors: ["violet", "#8b5cf6"],
                  },
                  {
                    label: "Sick",
                    used: latest.sick_leaves_used || 0,
                    total: latest.sick_leaves_total || 0,
                    colors: ["blue", "#3b82f6"],
                  },
                  {
                    label: "Annual",
                    used: latest.annual_leaves_used || 0,
                    total: latest.annual_leaves_total || 0,
                    colors: ["teal", "#14b8a6"],
                  },
                ].map((l) => {
                  const pct = l.total
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { label: "Bank Name", value: employee.bank.bank_name },
                {
                  ...
                  value: employee.bank.account_number,
                },
                {
                  label: "Account Title",
                  value: employee.bank.account_title_name,
                },
              ].map((f) => (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <ul className="text-xs text-amber-700 mt-2 space-y-1.5 list-disc list-inside">
                <li>
                  <strong>Daily Rate</strong> = Base Salary ÷ 30 (always divided
                  by 30, regardless of month having 28, 29, or 31 days)
                </li>
                <li>
                  <strong>Absent Deduction</strong> = Unpaid absent days × Daily
                  Rate (approved leaves are NOT deducted)
                </li>
                <li>
                  <strong>Late Deduction</strong> = Every 3 late arrivals = 1
                  day deducted
                </li>
                <li>
                  <strong>Salary Issuance</strong> = Around the 5th of the next
                  month (e.g., Feb salary issued ~5th March)
                </li>
                <li>
                  <strong>Net Salary</strong> = Base Salary + Allowances + Bonus
