...
const EmployeePersonalProfileV2 = ({ employeeId: propsEmployeeId, onBack }) => {
  ...
  const fetchEmployeeProfile = async (empId) => {
    try {
      ...
      const profileResponse =
        await EmployeeProfileService.getProfileSummary(empId);
      ...
      documents = profileData?.documents_json
        ? safeJsonParse(profileData.documents_json)
        : [];
      resources = profileData?.resources_json
        ? safeJsonParse(profileData.resources_json)
        : [];
      ...
      const [
        ...
        fetch(`${API_BASE_URL}/employees/${empId}`)
          .then((r) => r.json())
          .catch((e) => ({ success: false })),
        // 2. Financial summary
        EmployeeProfileService.getFinancialSummary(empId).catch((e) => ({
          success: false,
        })),
        // 3. Dynamic resources
        EmployeeProfileService.getEmployeeResources(empId).catch((e) => ({
          success: false,
        })),
        // 4. Skills
        EmployeeProfileService.getEmployeeSkills(empId).catch((e) => ({
          success: false,
        })),
        // 5. Social links
        EmployeeProfileService.getEmployeeSocialLinks(empId).catch((e) => ({
          success: false,
        })),
        // 6. Required documents
        EmployeeProfileService.getEmployeeRequiredDocuments(empId).catch(
          (e) => ({ success: false }),
        ),
        // 7. Achievements
        EmployeeProfileService.getEmployeeAchievements(empId).catch((e) => ({
          success: false,
        })),
      ]);
      ...
      if (onboardingResult.status === "fulfilled") {
        ...
        if (onboardingJson?.success)
          onboardingDetails = onboardingJson.data || {};
      }

      // Process financial summary
      if (
        finResult.status === "fulfilled" &&
        finResult.value?.success &&
      ...
      if (
        ...
        skillsResult.value?.success &&
        skillsResult.value?.data
      ) {
        skillsData = {
          technical: Array.isArray(skillsResult.value.data.technical)
            ? skillsResult.value.data.technical
            : [],
          soft: Array.isArray(skillsResult.value.data.soft)
            ? skillsResult.value.data.soft
            : [],
        };
      ...
      if (
        ...
        docsResult.value?.success &&
        docsResult.value?.data
      ) {
        requiredDocumentsData = Array.isArray(docsResult.value.data)
          ? docsResult.value.data
          : [];
      } else if (profileData?.required_documents_json) {
        try {
          requiredDocumentsData = JSON.parse(
            profileData.required_documents_json,
          );
        } catch (parseErr) {
      ...
      if (
        achResult.status === "fulfilled" &&
        achResult.value?.success &&
        achResult.value?.data
      ) {
        achievementsData = Array.isArray(achResult.value.data)
          ? achResult.value.data
          : [];
      ...
      const dynamicResourcesClean = Array.isArray(dynamicResources)
        ? dynamicResources.filter(
            (r) => r && (r.resource_name || r.name || r.title),
          )
        : [];

      // Normalize profile/onboarding fallback resources and filter out empty entries
      const profileResourcesArray = Array.isArray(resources)
        ? resources.slice()
        : resources
          ? [resources]
          : [];
      const profileResourcesClean = profileResourcesArray.filter(
        (item) =>
          item &&
      ...
      const onboardingResourcesArray = Array.isArray(
        onboardingDetails.resources,
      )
        ? onboardingDetails.resources.slice()
        : onboardingDetails.resources
          ? [onboardingDetails.resources]
          : [];
      const onboardingResourcesClean = onboardingResourcesArray.filter(
        (item) =>
          item &&
          Object.values(item).some(
            (v) =>
      ...
      const finalFallbackResources = profileResourcesClean.length
        ? profileResourcesClean
        : onboardingResourcesClean.length
          ? onboardingResourcesClean
          : [];

      const resourcesSource =
        dynamicResourcesClean.length > 0
          ? "employee_dynamic_resources"
          : finalFallbackResources.length
            ? "profile_resources"
            : null;
      const bankParts = (
        profileData?.bank_accounts_summary ||
        onboardingDetails.bank_account ||
        ""
      ).split("|");
      const transformedEmployee = {
        ...
        basicInfo: {
          ...
          profileImage:
            profileData?.profile_photo || profileData?.profile_image || null,
        },
        ...
        professionalInfo: {
          ...
          employmentStatus:
            profileData?.employment_status || profileData?.status || null,
          employeeType:
            profileData?.employee_type || profileData?.status || null,
        ...
        bankInfo: {
          accountNumber:
            financialSummary?.bank_account?.account_number_masked ||
            (onboardingDetails?.bank_account
              ? `****${(onboardingDetails.bank_account || "").slice(-4)}`
              : null),
          bankName:
            financialSummary?.bank_account?.bank_name ||
            onboardingDetails?.bank_name ||
            null,
          accountType:
            financialSummary?.bank_account?.account_type ||
            onboardingDetails?.account_type ||
            null,
          accountTitle:
            financialSummary?.bank_account?.account_title_name ||
            onboardingDetails?.account_title_name ||
            null,
        },
        ...
        financialInfo: {
          ...
          baseSalary: financialSummary?.base_salary
            ? Number(financialSummary.base_salary)
        ...
        resources:
          dynamicResourcesClean.length > 0
            ...
            : finalFallbackResources.length
              ? finalFallbackResources
              : [],
        resourcesSource: resourcesSource,
        resourcesMessage: resourcesMessage,
        achievements: achievementsData,
        profileCompleteness:
          profileResponse.data?.metadata?.profile_completeness || 0,
        ...
        personalIdentifiers: {
          ...
          cnicIssueDate:
            onboardingDetails.cnic_issue_date ||
            onboardingDetails.cnicIssueDate ||
            null,
          cnicExpiryDate:
            onboardingDetails.cnic_expiry_date ||
            onboardingDetails.cnicExpiryDate ||
            null,
        },
      };

      // Normalize arrays to avoid runtime errors when backend returns single objects or unexpected types
      transformedEmployee.documents = Array.isArray(
        transformedEmployee.documents,
      )
        ? transformedEmployee.documents
        : transformedEmployee.documents
          ? [transformedEmployee.documents]
          : [];
      transformedEmployee.resources = Array.isArray(
        transformedEmployee.resources,
      )
        ? transformedEmployee.resources
        : transformedEmployee.resources
          ? [transformedEmployee.resources]
          : [];
      transformedEmployee.achievements = Array.isArray(
        transformedEmployee.achievements,
      )
        ? transformedEmployee.achievements
        : transformedEmployee.achievements
          ? [transformedEmployee.achievements]
          : [];
      transformedEmployee.requiredDocuments = Array.isArray(
        transformedEmployee.requiredDocuments,
      )
        ? transformedEmployee.requiredDocuments
        : transformedEmployee.requiredDocuments
          ? [transformedEmployee.requiredDocuments]
          : [];
      transformedEmployee.socialLinks =
        transformedEmployee.socialLinks &&
        typeof transformedEmployee.socialLinks === "object"
          ? transformedEmployee.socialLinks
          : {};
      transformedEmployee.financialInfo =
        transformedEmployee.financialInfo || {};
      transformedEmployee.financialInfo.allowancesList = Array.isArray(
        transformedEmployee.financialInfo.allowancesList,
      )
        ? transformedEmployee.financialInfo.allowancesList
        : transformedEmployee.financialInfo.allowancesList
          ? [transformedEmployee.financialInfo.allowancesList]
      ...
      transformedEmployee.skills = transformedEmployee.skills || {
        technical: [],
        soft: [],
      };
      transformedEmployee.skills.technical = Array.isArray(
        transformedEmployee.skills.technical,
      )
        ? transformedEmployee.skills.technical
        : transformedEmployee.skills.technical
          ? [transformedEmployee.skills.technical]
          : [];
      transformedEmployee.skills.soft = Array.isArray(
        transformedEmployee.skills.soft,
      )
        ? transformedEmployee.skills.soft
        : transformedEmployee.skills.soft
          ? [transformedEmployee.skills.soft]
    ...
    try {
      ...
      const profilePayload = {
        ...
        emergency_contact_phone:
          employee.personalDetails?.emergencyContactPhone || null,
        preferred_contact_method:
          employee.profile?.preferredContactMethod || null,
      ...
      if (employee?.basicInfo?.email)
        onboardingPayload.email = employee.basicInfo.email;
      if (employee?.basicInfo?.phone)
        onboardingPayload.phone = employee.basicInfo.phone;
      if (employee?.basicInfo?.personalPhone)
        onboardingPayload.personal_phone = employee.basicInfo.personalPhone;
      if (employee?.personalDetails?.address)
        onboardingPayload.address = employee.personalDetails.address;
      ...
      if (Object.keys(onboardingPayload).length > 0) {
        ...
        if (!res.ok || !json.success) {
          throw new Error(
            json.message || "Failed to update onboarding (contact info)",
          );
        }
      }
      ...
      const socialLinksFromPersonal = {
        linkedin: employee.personalDetails?.linkedin
          ? { url: employee.personalDetails.linkedin }
          : null,
        github: employee.personalDetails?.github
          ? { url: employee.personalDetails.github }
          : null,
        portfolio: employee.personalDetails?.portfolio
          ? { url: employee.personalDetails.portfolio }
          : null,
      };
      ...
      if (Object.keys(socialLinksToSave).length > 0) {
        console.log(
          "Updating social links from personalDetails:",
          socialLinksToSave,
        );
        const socialLinksResult =
          await EmployeeProfileService.updateEmployeeSocialLinks(
            employee.id,
            socialLinksToSave,
          );
        if (!socialLinksResult.success) {
          console.warn(
            "Warning: Social links update may have failed:",
      ...
      if (
        ...
        if (Object.keys(filteredAdditionalLinks).length > 0) {
          console.log(
            "Updating additional social links:",
            filteredAdditionalLinks,
          );
          const additionalSocialLinksResult =
            await EmployeeProfileService.updateEmployeeSocialLinks(
              employee.id,
              filteredAdditionalLinks,
            );
          if (!additionalSocialLinksResult.success) {
            console.warn(
              "Warning: Additional social links update may have failed:",
              additionalSocialLinksResult.message,
            );
          }
        }
      }
      ...
      if (
        ...
        if (validAchievements.length > 0) {
          ...
          const normalizedAchievements = validAchievements.map(
            normalizeAchievements,
          );
          console.log(
            "Updating achievements (normalized):",
            normalizedAchievements,
          );
          const achievementsResult =
            await EmployeeProfileService.updateEmployeeAchievements(
              employee.id,
              normalizedAchievements,
            );
          if (!achievementsResult.success) {
            console.warn(
              "Warning: Achievements update may have failed:",
      ...
      if (
        ...
        Array.isArray(employee.requiredDocuments) &&
        employee.requiredDocuments.length > 0
      ) {
        // Only keep documents with at least a name/type and some meaningful data
        const validDocuments = employee.requiredDocuments.filter(
          (doc) =>
            doc &&
        ...
        if (validDocuments.length > 0) {
          ...
          const docsResult =
            await EmployeeProfileService.updateEmployeeRequiredDocuments(
              employee.id,
              validDocuments,
            );
          if (!docsResult.success) {
            console.warn(
              "Warning: Required documents update may have failed:",
              docsResult.message,
            );
          }
        }
  ...
  const uploadBanner = async () => {
    ...
    try {
      ...
      const result = await EmployeeProfileService.uploadBanner(
        employee.id,
        bannerFile,
      );
  ...
  const uploadProfile = async () => {
    ...
    try {
      ...
      const result = await EmployeeProfileService.uploadProfilePhoto(
        employee.id,
        profileFile,
      );
      ...
      try {
        ...
        setEmployee((prev) => ({
          ...
          basicInfo: {
            ...prev.basicInfo,
            profileImage: result.data.profile_photo_url,
          },
        }));
      }
      setAvatarKey(Date.now());
  ...
  const uploadDocument = async () => {
    ...
    try {
      ...
      const result = await EmployeeProfileService.uploadDocuments(
        employee.id,
        documentsToUpload,
      );
      ...
      const uploadedDoc = {
        ...
        url:
          result.data.documents && result.data.documents[0]
            ? result.data.documents[0].url
  ...
  const uploadRequiredDocument = async () => {
    ...
    try {
      ...
      const result = await EmployeeProfileService.uploadRequiredDocuments(
        employee.id,
        documentsToUpload,
      );
  ...
  const normalizeAchievements = (ach) => ({
    ...
    credential_url:
      ach.credential_url || ach.credentialUrl || ach.credential || null,
    attachment_url:
      ach.attachment_url || ach.attachmentUrl || ach.attachment || null,
    is_verified: typeof ach.is_verified !== "undefined" ? ach.is_verified : 0,
  });

  const saveAchievements = async (achArray) => {
    const normalized = (achArray || [])
      .map(normalizeAchievements)
      .filter((a) => a.title && a.title.trim());
    try {
      console.log(
        "🏆 [saveAchievements] Saving normalized achievements:",
        normalized,
      );
      const result = await EmployeeProfileService.updateEmployeeAchievements(
        employee.id,
        normalized,
      );
      if (result.success) {
        ...
        try {
          await fetchEmployeeProfile(employee.id);
          console.log(
            "🔄 [saveAchievements] Refreshed profile after achievements save",
          );
        } catch (e) {
          console.warn(
            "⚠️ [saveAchievements] Failed to refresh profile:",
            e?.message || e,
          );
        }
      } else {
        console.warn(
          "⚠️ [saveAchievements] Backend returned failure:",
          result.message,
        );
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (employeeId) {
      ...
      setError(
        "No employee selected. Please select an employee from the list.",
      );
      setLoading(false);
    }
  }, [employeeId]);
  ...
  return (
    <ProtectedModule
      ...
      description="Personal and professional information about the employee. Access requires security verification."
    >
        ...
        <div className="max-w-full mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mb-8">
            <div
              ...
              style={
                employee?.profile?.bannerImage
                  ? {
                      backgroundImage: `url(${employee?.profile?.bannerImage})`,
                    }
                  : {}
              }
            >
                ...
                <div className="flex justify-end p-3 items-center gap-3">
                  ...
                  {bannerFile && (
                    <div className="inline-flex items-center gap-2">
                      ...
                      >
                        {isUploadingBanner ? "Uploading..." : "Apply"}
                      </button>
                      <button
                        onClick={() => {
                          setBannerFile(null);
                          updateEmployeeField("profile.bannerImage", null);
                        }}
              ...
              <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 -mt-20 relative z-10">
                <div className="flex items-end gap-6">
                  ...
                  <div className="relative">
                    <div className="w-32 h-32 rounded-2xl border-4 border-white shadow-xl bg-gradient-to-br from-blue-200 to-purple-200 flex items-center justify-center">
                      {employee?.basicInfo?.profileImage ? (
                        <img
                          src={
                            employee?.basicInfo?.profileImage
                              ? `${employee.basicInfo.profileImage}${avatarKey ? `?v=${avatarKey}` : ""}`
                              : undefined
                          }
                          alt={employee?.basicInfo?.name || "Profile"}
                    ...
                    <label
                      ...
                      className="absolute right-2 bottom-2 inline-flex items-center bg-white p-0.5 rounded-full cursor-pointer shadow-lg z-10"
                    >
                      <input
                        id="profile-upload"
                        type="file"
                    ...
                    {profileFile && (
                      <div className="mt-3 flex items-center gap-2">
                        <button
                          ...
                          className="px-3 py-1 bg-green-600 text-white rounded-md text-sm"
                        >
                          {isUploadingProfile ? "Uploading..." : "Apply"}
                        </button>
                        <button
                          onClick={() => {
                            setProfileFile(null);
                  ...
                  <div className="flex-1 pb-2">
                    <h1 className="text-3xl font-bold text-white mb-1">
                      {employee?.basicInfo?.name || "Unnamed"}
                    </h1>
                ...
                <div className="flex flex-wrap gap-3">
                  {!isEditing ? (
                    <>
                      <button
                        className="p-3 hover:bg-gray-100 rounded-lg border border-gray-200 transition-all"
                        aria-label="Print"
                      >
                ...
                {isEditing && (
                  <p className="text-xs text-gray-500 mt-3">
                    Editable sections: <strong>Contact Information</strong>,{" "}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
            ...
            <div className="lg:col-span-1 space-y-6">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        value={employee?.basicInfo?.email || ""}
                        onChange={(e) =>
                          updateEmployeeField("basicInfo.email", e.target.value)
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={employee?.basicInfo?.phone || ""}
                        onChange={(e) =>
                          updateEmployeeField("basicInfo.phone", e.target.value)
                        }
                        className="mt-2 block w-full rounded-md border border-gray-200 py-2 px-3 bg-white text-sm"
                      />
                    </div>
              ...
              >
                ...
                <InfoRow
                  label="Account"
                  value={`****${(employee?.bankInfo?.accountNumber || "").slice(-4) || "****"}`}
                  copyable
                  subtitle="Masked for security"
                />
                <InfoRow
                  label="Account Type"
                  value={employee?.bankInfo?.accountType || "—"}
                />
                <InfoRow
                  label="Account Title"
                  value={employee?.bankInfo?.accountTitle || "—"}
                />
              </CardSection>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        type="text"
                        value={
                          employee?.personalDetails?.emergencyContactPhone || ""
                        }
                        onChange={(e) =>
                          updateEmployeeField(
              ...
              >
                <div className="space-y-4">
                  ...
                  {!canEdit("social") ? (
                    ...
                    employee?.socialLinks &&
                    Object.keys(employee.socialLinks).length > 0 ? (
                      Object.entries(employee.socialLinks).map(
                        ([platform, data]) => {
                          console.log(
                          ...
                          if (!url || !url.trim()) {
                            console.log(
                              `⚠️ [Social Link Item] No URL for ${platform}`,
                            );
                            return null;
                          }
                      ...
                      <>
                        {console.log(
                          "❌ [Social Links] Empty or no socialLinks object",
                        )}
                                  ...
                                  >
                                    ...
                                    <button
                                      onClick={async () => {
                                        ...
                                        console.log(
                                          `🗑️ [Remove Social Link] Removed ${platform} from state`,
                                        );
                                        ...
                                        try {
                                          ...
                                          if (result.success) {
                                            console.log(
                                              `✅ [Remove Social Link] ${platform} successfully removed from database`,
                                            );
                                            setSuccessMessage(
                                              `${platform} link removed successfully!`,
                                            );
                                            setTimeout(
                                            ...
                                            console.error(
                                              `❌ [Remove Social Link] Failed to remove ${platform}`,
                                              result.message,
                                            );
                                            // Revert on failure
                                            updateEmployeeField(
                          ...
                          <button
                            onClick={() => {
                              if (
                                ...
                                toast.error(
                                  "Please enter both platform name and URL",
                                );
                                return;
                              }
                              const updated = { ...employee.socialLinks };
                              updated[newSocialLink.platform] = {
                                url: newSocialLink.url,
                              };
                              updateEmployeeField("socialLinks", updated);
                              setNewSocialLink({ platform: "", url: "" });
                              console.log(
                                `✅ Added social link: ${newSocialLink.platform}`,
                              );
                            }}
                    ...
                    {canEdit("documents") && (
                      <div className="space-y-3 mb-4 pb-4 border-b">
                        ...
                        <div className="flex gap-2">
                          <input
                            type="file"
                            onChange={handleRequiredDocFileChange}
                            className="border rounded-md p-2 text-sm bg-white flex-1"
                          />
                          <button
                            onClick={uploadRequiredDocument}
                            disabled={isUploadingReqDoc}
                            className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm whitespace-nowrap"
                          >
                            {isUploadingReqDoc ? "Uploading..." : "Upload"}
                          </button>
                        </div>
                        {requiredDocFile && (
                          <p className="text-xs text-gray-600">
                            Selected: {requiredDocFile.name}
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    <div className="space-y-2">
                      ...
                      employee.requiredDocuments.length > 0 ? (
                        employee.requiredDocuments.map((doc, idx) => (
                          <div
                            key={doc.id || idx}
                            className="flex items-center justify-between p-3 border rounded-md bg-green-50 border-green-200 hover:bg-green-100 transition"
                          >
                            <div className="flex-1 min-w-0">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <InfoRow
                    label="Employee ID"
                    value={employee?.basicInfo?.employeeId || "—"}
                  />
                  <InfoRow
                    label="Department"
                  ...
                  <InfoRow
                    label="Reporting To"
                    value={employee?.professionalInfo?.reportingTo || "—"}
                  />
                  <InfoRow
                    label="Joining Date"
                    value={formatDate(employee?.professionalInfo?.joiningDate)}
                  />
                </div>
              </CardSection>
              ...
              >
                <div className="space-y-3">
                  <InfoRow
                    label="Base Salary"
                    value={`${employee?.financialInfo?.baseSalary != null ? Number(employee.financialInfo.baseSalary).toLocaleString() : "—"} PKR`}
                  />
                  <div className="pt-2">
                    ...
                    <div className="mt-2 space-y-2">
                      {(Array.isArray(employee?.financialInfo?.allowancesList)
                        ? employee.financialInfo.allowancesList
                        : []
                      ).map((a, i) => (
                        <div
                          key={`allowance-${i}-${a?.name}`}
                          className="flex items-center justify-between p-2 bg-gray-50 rounded-md border border-gray-100"
                        >
                          <span className="text-sm">{a?.name || "—"}</span>
                          <span className="text-sm font-semibold text-blue-600">
                            {(Number(a?.amount) || 0).toLocaleString()} PKR
                          </span>
                        </div>
                      ))}
                    </div>
                    <div className="mt-2 flex items-center justify-between p-2 bg-blue-50 rounded-md border border-blue-100 font-semibold">
                      ...
                      <span className="text-blue-600">
                        {employee?.financialInfo?.totalAllowances != null
                          ? Number(
                              employee.financialInfo.totalAllowances,
                            ).toLocaleString()
                          : "0"}{" "}
                        PKR
                      </span>
              ...
              >
                <div className="space-y-4">
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        ...
                        {resourceType === "equipment" && (
                          <input
                            type="text"
                            value={resourceSerial}
                            onChange={(e) => setResourceSerial(e.target.value)}
                      ...
                      <div className="flex items-center gap-3">
                        <input
                          type="date"
                          value={resourceIssuedAt}
                          onChange={(e) => setResourceIssuedAt(e.target.value)}
                          className="border rounded-md p-2"
                        />
                        <button
                          onClick={addResource}
                        ...
                        <p className="text-sm text-gray-500">
                          Equipment will be tracked by serial number.
                        </p>
                      </div>
                    </>
                  )}
                  {!canEdit("resources") && (
                    <p className="text-sm text-gray-500">
                      Resources are loaded from the `employee_dynamic_resources`
                      table. Contact IT/HR to change allocations.
                    </p>
                  )}
                  ...
                  <div className="space-y-2">
                    {(() => {
                      const resourcesArray = Array.isArray(employee?.resources)
                        ? employee.resources
                        : [];
                      const visibleResources = resourcesArray.filter(
                        (r) => r && (r.resource_name || r.title || r.name),
                      );

                      if (visibleResources.length > 0) {
                        return (
                          <>
                            {employee?.resourcesSource ===
                              "employee_dynamic_resources" && (
                                ...
                                <div className="flex-1">
                                  ...
                                  <p className="text-xs text-gray-600 mt-1">
                                    ...
                                    <span className="font-mono text-gray-700">
                                      {r.resource_serial || r.serial || "N/A"}
                                    </span>
                                  </p>
                      ...
                      return (
                        <div className="text-sm text-gray-500 italic">
                          ...
                          <p className="text-xs text-gray-500">
                            No resources found — please contact{" "}
                            <span className="font-medium text-gray-700">
                              IT/HR
                            </span>{" "}
                            to request allocations.
                          </p>
                        </div>
                      );
                    })()}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      {(Array.isArray(employee?.skills?.technical)
                        ? employee.skills.technical
                        : []
                      ).map((skill, i) => (
                        <div
                          key={`tech-skill-${i}-${skill}`}
                    ...
                    {canEdit("skills") && (
                      <div className="mt-3 flex gap-2">
                        <input
                          ...
                          className="border rounded-md p-1 text-sm w-48"
                        />
                        <button
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      {(Array.isArray(employee?.skills?.soft)
                        ? employee.skills.soft
                        : []
                      ).map((skill, i) => (
                        <div
                          key={`soft-skill-${i}-${skill}`}
                          className="flex items-center gap-2"
                        >
              ...
              <div className="space-y-4">
                ...
                employee?.achievements?.length > 0 ? (
                  employee?.achievements?.map((ach, i) => {
                    console.log(
                      `🏆 [Achievements Display] Achievement ${i}:`,
                      ach,
                    );
                    return (
                      <div
                        key={`achievement-${i}-${ach?.id || ach?.title}`}
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-700 font-medium">
                              ...
                              {ach?.issuer_organization || ach?.issuer || "—"}
                            </p>
                            <p className="text-xs text-gray-600 mt-2">
                              {ach?.issue_date
                                ? `Issued: ${new Date(ach.issue_date).toLocaleDateString()}`
                                : "No issue date"}
                              {ach?.expiry_date &&
                                ` • Expires: ${new Date(ach.expiry_date).toLocaleDateString()}`}
                            </p>
                            {ach?.description && (
                          ...
                          {canEdit("achievements") && (
                            <div className="ml-4">
                              <button
                                ...
                                className="px-2 py-1 text-red-600 bg-red-50 text-xs rounded hover:bg-red-100"
                              >
                                Remove
                              </button>
                            </div>
                  ...
                  <p className="text-sm text-gray-500 italic">
                    No achievements listed yet. Start adding your certifications
                    and accomplishments!
                  </p>
                )}
                  ...
                  <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <input
                        ...
                        value={newAchievement.expiryDate}
                        onChange={(e) =>
                          setNewAchievement((prev) => ({
                            ...prev,
...
const SocialLink = ({ icon, label, url }) => (
  <a
    ...
    target="_blank"
    rel="noopener noreferrer"
    className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-blue-50 border border-gray-200 hover:border-blue-200 transition-all group"
  >
    <div className="flex items-center gap-3">
