...
const HRAttendanceSync = () => {
  ...
  const checkOngoingSync = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://http://100.114.9.93:5000'}/api/v1/zkTime/sync-jobs`, {
        headers: {
  ...
  const handleSyncAll = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://http://100.114.9.93:5000'}/api/v1/zkTime/sync-all`, {
        method: 'POST',
        headers: {
  ...
  const startPolling = (jobId) => {
    let interval = setInterval(async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://http://100.114.9.93:5000'}/api/v1/zkTime/sync-status/${jobId}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
