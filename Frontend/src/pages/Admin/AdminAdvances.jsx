...
const STATUS_COLORS = { active: "bg-emerald-100 text-emerald-700", completed: "bg-slate-100 text-slate-600", cancelled: "bg-rose-100 text-rose-700" };
...
const AdminAdvances = () => {
  ...
  // Form
  const defaultForm = { employee_id: "", type: "advance", amount: "", repayment_months: "1", start_date: new Date().toISOString().slice(0, 7) + "-01", notes: "" };
  ...
  const filtered = advances.filter((a) => {
    ...
    return (!q || a.employee_name?.toLowerCase().includes(q) || a.employee_code?.toLowerCase().includes(q) || a.notes?.toLowerCase().includes(q));
  });
            ...
            <div className="bg-white rounded-2xl p-5 shadow border border-slate-200 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search by employee name or code…" value={searchQuery}
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[900px]">
                  <thead className="bg-slate-100">
                    <tr>
                      {["Employee", "Type", "Amount", "Monthly Ded.", "Progress", "Status", "Start", "Actions"].map(h => (
                    ...
                    {loading ? (
                      ...
                      return (
                        <tr key={a.id} className="hover:bg-slate-50 transition">
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-2">
                              <div className="flex-1 h-2 bg-slate-200 rounded-full overflow-hidden">
                                <div className={`h-full rounded-full transition-all ${a.status === 'completed' ? 'bg-emerald-500' : a.status === 'cancelled' ? 'bg-rose-400' : 'bg-blue-500'}`}
                          ...
                          <td className="px-4 py-4">
                            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${STATUS_COLORS[a.status] || "bg-slate-100"}`}>
                              {a.status}
                            </span>
                          </td>
                          <td className="px-4 py-4 text-sm text-slate-600">{formatDate(a.start_date)}</td>
                          <td className="px-4 py-4">
                            <div className="flex gap-1">
                              ...
                              {a.status === "active" && (
                                <button onClick={() => handleCancel(a.id)} className="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition" title="Cancel"><XCircle size={15} /></button>
                              )}
                              <button onClick={() => handleDelete(a.id)} className="p-2 text-rose-600 hover:bg-rose-50 rounded-lg transition" title="Delete"><Trash2 size={15} /></button>
                            </div>
                          </td>
                        </tr>
                      );
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4"
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl max-h-[90vh] overflow-y-auto" onMouseDown={e => e.stopPropagation()}>
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-5">
          ...
          {/* Repayment Months */}
          <div>
            <label className="block text-gray-700 font-medium mb-2">Repayment Period (months) <span className="text-blue-600">*</span></label>
          ...
          {monthlyPreview > 0 && (
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm font-semibold text-blue-800">Monthly Deduction Preview</span>
              </div>
              <p className="text-2xl font-bold text-blue-600">{formatCurrency(monthlyPreview)} <span className="text-sm font-normal text-blue-500">/ month</span></p>
              <p className="text-xs text-blue-500 mt-1">
                {formatCurrency(form.amount)} over {form.repayment_months} month{form.repayment_months > 1 ? 's' : ''}
          ...
          {/* Start Date */}
          <div>
            <label className="block text-gray-700 font-medium mb-2">Start Date <span className="text-blue-600">*</span></label>
            <input type="date" value={form.start_date}
              onChange={e => setForm({ ...form, start_date: e.target.value })}
              className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500"
              required />
            <p className="text-xs text-slate-400 mt-1">First deduction will apply to this month's payroll</p>
          </div>

          {/* Notes */}
          <div>
            <label className="block text-gray-700 font-medium mb-2">Notes</label>
            <textarea placeholder="Reason, purpose, or additional details…" value={form.notes}
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="h-3 bg-slate-200 rounded-full overflow-hidden">
              <div className={`h-full rounded-full transition-all ${advance.status === 'completed' ? 'bg-emerald-500' : advance.status === 'cancelled' ? 'bg-rose-400' : 'bg-blue-500'}`}
                style={{ width: `${Math.min(progress, 100)}%` }} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 rounded-xl">
            <div><p className="text-xs text-slate-500">Status</p>
              <span className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${STATUS_COLORS[advance.status]}`}>{advance.status}</span>
            </div>
            ...
            <div><p className="text-xs text-slate-500">Start Date</p><p className="font-semibold text-slate-800 text-sm">{formatDate(advance.start_date)}</p></div>
                    ...
                    {advance.installments.map((inst) => {
                      ...
                      return (
                        <tr key={inst.id} className={`hover:bg-slate-50 ${inst.status === 'deducted' ? 'bg-emerald-50/50' : inst.status === 'skipped' ? 'bg-slate-50 opacity-50' : ''}`}>
                          ...
                          <td className="px-4 py-3 text-sm font-medium text-slate-800">{monthNames[inst.month]} {inst.year}</td>
