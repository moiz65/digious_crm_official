...
import {
  ...
  CheckCircle, XCircle, LayoutList, Check,
} from "lucide-react";
...
const AdminExpense = () => {
  ...
  const fetchExpenses = useCallback(async () => {
    ...
    try {
      ...
      else { setError(data.message || "Failed to load expenses"); setExpenses([]); }
    } catch (err) {
  ...
  const handleSubmitCategory = async (e) => {
    ...
    try {
      const body = { name: catForm.name.trim(), description: catForm.description, color: selectedCat ? selectedCat.color : getRandomColor() };
      const data = selectedCat
        ? await apiFetch(`${API_BASE}/expenses/categories/${selectedCat.id}`, { method: "PUT",  body: JSON.stringify(body) })
        : await apiFetch(`${API_BASE}/expenses/categories`,                    { method: "POST", body: JSON.stringify(body) });
      if (data.success) { await fetchCategories(); closeCatModal(); }
  ...
  const handleToggleCategoryActive = async (cat) => {
    const data = await apiFetch(`${API_BASE}/expenses/categories/${cat.id}`, {
      method: "PUT", body: JSON.stringify({ is_active: cat.is_active ? 0 : 1 }),
    });
  ...
  const closeCatModal = () => { setShowCatModal(false); setSelectedCat(null); setCatForm({ name: "", description: "" }); setCatError(null); };
  ...
  const thisMonthTotal  = expenses.filter(e => e.expense_date?.startsWith(thisMonthPfx)).reduce((s, e) => s + parseFloat(e.amount || 0), 0);
  const lastMonthTotal  = expenses.filter(e => e.expense_date?.startsWith(lastMonthPfx)).reduce((s, e) => s + parseFloat(e.amount || 0), 0);
  const todayTotal      = expenses.filter(e => e.expense_date === today).reduce((s, e) => s + parseFloat(e.amount || 0), 0);
  const yesterdayTotal  = expenses.filter(e => e.expense_date === yesterdayStr).reduce((s, e) => s + parseFloat(e.amount || 0), 0);
  ...
  return (
    <div className="flex h-screen bg-gray-50">
          ...
          <div className="p-8 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-slate-500 text-sm flex items-center gap-2">
                  ...
                  {formatCurrency(totalExpenses)} total &bull; {filteredExpenses.length} records &bull; {categories.filter(c => c.is_active).length} active categories
                </p>
              </div>
              <div className="flex items-center gap-3">
                <button onClick={fetchExpenses} className="flex items-center gap-2 px-4 py-2.5 border rounded-xl font-semibold bg-white border-blue-200 text-blue-600 hover:bg-blue-50 transition">
                ...
                {activeTab === "expenses" ? (
                  ...
                  <button onClick={() => { setSelectedCat(null); setCatForm({ name: "", description: "", color: "#3B82F6" }); setShowCatModal(true); }}
                    className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition hover:scale-105">
            ...
            <div className="flex gap-1 bg-white rounded-2xl p-1 shadow border border-slate-200 mb-6 w-fit">
              ...
              >
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${activeTab === "categories" ? "bg-white/20 text-white" : "bg-blue-100 text-blue-600"}`}>
                  {categories.length}
                </span>
              </button>
            </div>
            ...
            {activeTab === "expenses" && (
              <>
                        ...
                        <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
                          {suggestions.map((s, i) => (
                            <button
                              ...
                              onMouseDown={() => { setSearchQuery(s.value); setShowSuggestions(false); }}
                            >
                              {s.type === "category" ? (
                                <>
                                  <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: s.color || "#6366f1" }} />
                                ...
                                <>
                                  ...
                                  <span className="ml-auto text-xs text-slate-400 bg-blue-50 px-2 py-0.5 rounded-full">Expense</span>
                                </>
                              )}
                            </button>
                          ))}
                ...
                {/* Expense Table */}
                <ExpenseTable
                  data={filteredExpenses} loading={loading} filteredTotal={filteredTotal}
                  onView={(e) => { setSelectedExpense(e); setShowViewModal(true); }}
              ...
              <>
                {catError && (
                  <div className="mb-4 p-4 bg-rose-50 border border-rose-200 rounded-xl text-rose-700 flex items-center gap-3 text-sm">
                    ...
                    <button onClick={() => setCatError(null)} className="ml-auto"><X className="h-4 w-4" /></button>
                  </div>
                )}
                ...
                {catSuccess && (
                  <div className="mb-4 p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-700 flex items-center gap-3 text-sm">
                    ...
                    <button onClick={() => setCatSuccess(null)} className="ml-auto"><X className="h-4 w-4" /></button>
                  </div>
                )}
                ...
                <div className="bg-white rounded-2xl shadow border border-slate-200 overflow-hidden">
                      ...
                      <thead className="bg-slate-100 border-b border-slate-200">
                        <tr>
                          {["Category", "Description", "Status", "Actions"].map(h => (
                            <th key={h} className="px-6 py-3 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {filteredCats.length === 0 ? (
                          <tr><td colSpan="4" className="py-12 text-center">
                            ...
                            <p className="text-sm text-slate-400">{catSearch ? "Try a different search." : `Click "Add Category" to create one.`}</p>
                          </td></tr>
                        ) : filteredCats.map(cat => (
                          <tr key={cat.id} className={`hover:bg-slate-50 transition ${!cat.is_active ? "opacity-50" : ""}`}>
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold text-sm shadow flex-shrink-0"
                            ...
                            <td className="px-6 py-4 text-sm text-slate-600">{cat.description || <span className="text-slate-400 italic">—</span>}</td>
                            <td className="px-6 py-4">
                              <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${cat.is_active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
                                {cat.is_active ? "Active" : "Inactive"}
                              </span>
                            </td>
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2">
                                <button onClick={() => handleToggleCategoryActive(cat)} title={cat.is_active ? "Deactivate" : "Activate"}
                                  ...
                                  {cat.is_active ? <XCircle size={16} /> : <CheckCircle size={16} />}
                                </button>
                  ...
                  {filteredCats.length > 0 && (
                    <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 text-sm text-slate-600 flex gap-6">
                      <span><strong className="text-slate-800">{categories.filter(c => c.is_active).length}</strong> active</span>
                      <span><strong className="text-slate-800">{categories.filter(c => !c.is_active).length}</strong> inactive</span>
                      <span><strong className="text-slate-800">{categories.length}</strong> total</span>
                    </div>
                  )}
                </div>
              </>
      ...
      {showExpenseModal && (
        <ExpenseModal
          expense={selectedExpense} formData={expenseForm} setFormData={setExpenseForm}
          categories={categories} saving={saving} error={error}
          onClose={closeExpenseModal} onSubmit={handleSubmitExpense}
        />
      )}
      ...
      {showViewModal && selectedExpense && (
        <ViewExpenseModal
          ...
          onEdit={() => { setShowViewModal(false); openEditExpense(selectedExpense); }}
        />
      )}
      ...
      {showCatModal && (
        <CategoryModal
          category={selectedCat} formData={catForm} setFormData={setCatForm}
          saving={catSaving} error={catError}
          onClose={closeCatModal} onSubmit={handleSubmitCategory}
        />
      )}
    </div>
  );
          ...
          <tbody className="divide-y divide-slate-100">
            {data.length === 0 ? (
              <tr><td colSpan="5" className="py-16 text-center">
                ...
                <p className="text-slate-500 font-medium">No expense records</p>
                <p className="text-sm text-slate-400">Click "Add Expense" to get started.</p>
              </td></tr>
            ) : data.map(expense => (
              <tr key={expense.id} className="hover:bg-slate-50 transition">
                <td className="px-4 py-4">
                  <div className="font-semibold text-blue-600 text-sm">{fmtExpenseId(expense.id)}</div>
                  <div className="text-xs text-slate-400">{formatDate(expense.expense_date)}{expense.expense_time ? ` • ${formatTime(expense.expense_time)}` : ""}</div>
                </td>
                <td className="px-4 py-4">
                  <span className="px-3 py-1 rounded-full text-xs font-semibold"
                    style={expense.category_color ? { background: expense.category_color + "22", color: expense.category_color } : { background: "#e5e7eb", color: "#374151" }}>
                    {expense.category_name || "—"}
                  </span>
                </td>
                <td className="px-4 py-4 font-bold text-blue-600 text-base">{formatCurrency(expense.amount)}</td>
                <td className="px-4 py-4 max-w-xs">
                  <span className="text-sm text-slate-600 line-clamp-2">{expense.note || <i className="text-slate-400">No note</i>}</span>
                </td>
                <td className="px-4 py-4">
                  <div className="flex gap-1">
                    <button onClick={() => onView(expense)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition"><Eye size={15} /></button>
                    <button onClick={() => onEdit(expense)} className="p-2 text-amber-600 hover:bg-amber-50 rounded-lg transition"><Edit size={15} /></button>
                    <button onClick={() => onDelete(expense.id)} className="p-2 text-rose-600 hover:bg-rose-50 rounded-lg transition"><Trash2 size={15} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {data.length > 0 && (
        <div className="border-t border-slate-200 bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-600">Total Filtered Amount</p>
              <p className="text-xs text-slate-400">{data.length} transactions</p>
            </div>
          </div>
...
// ─── Expense Add/Edit Modal ───────────────────────────────────────────────────
const ExpenseModal = ({ expense, formData, setFormData, categories, saving, error, onClose, onSubmit }) => {
  const [catSearch, setCatSearch] = React.useState("");
  const [showCatDropdown, setShowCatDropdown] = React.useState(false);
  const catInputRef = React.useRef(null);

  const filteredCats = categories.filter(c => c.is_active && c.name.toLowerCase().includes(catSearch.toLowerCase())).slice(0, 8);
  const selectedCatName = formData.category_id ? categories.find(c => c.id === +formData.category_id)?.name : "";

  React.useEffect(() => {
    const handler = (e) => {
      if (catInputRef.current && !catInputRef.current.contains(e.target)) setShowCatDropdown(false);
    };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  }, []);
    ...
    <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl" onMouseDown={e => e.stopPropagation()}>
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-5 rounded-t-2xl flex items-center justify-between">
        <div>
          ...
          <p className="text-blue-100 text-sm mt-0.5">{expense ? "Update expense details" : "Fill in the details to record"}</p>
        </div>
      ...
      <form onSubmit={onSubmit} className="p-6 space-y-5">
        ...
        <div ref={catInputRef}>
          <label className="block text-gray-700 font-medium mb-2">Category <span className="text-blue-600">*</span></label>
          <div className="relative">
            <input
              type="text"
              placeholder="Search or select category…"
              value={catSearch || selectedCatName}
              onChange={(e) => { setCatSearch(e.target.value); setFormData({ ...formData, category_id: "" }); setShowCatDropdown(true); }}
            ...
            {(catSearch || !formData.category_id) && (
              <button
                type="button"
                onClick={() => { setCatSearch(""); setFormData({ ...formData, category_id: "" }); setShowCatDropdown(false); }}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"
              >
                <X className="h-4 w-4" />
              </button>
              ...
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-xl z-50 overflow-hidden max-h-64 overflow-y-auto">
                {filteredCats.length === 0 ? (
                  ...
                  filteredCats.map(cat => (
                    ...
                    >
                      <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: cat.color || "#3B82F6" }} />
                      <span className="text-sm font-medium text-slate-700 flex-1">{cat.name}</span>
                      <span className="text-xs text-slate-400">{cat.description ? cat.description.slice(0, 20) : ""}</span>
                    </button>
                  ))
                )}
              </div>
        ...
        <div>
          <label className="block text-gray-700 font-medium mb-2">Amount <span className="text-blue-600">*</span></label>
          <div className="relative">
            <span className="absolute left-4 top-3 text-gray-500 font-medium text-sm">PKR</span>
        ...
        <div>
          ...
          <textarea placeholder="Add a note (optional)" value={formData.note} onChange={e => setFormData({ ...formData, note: e.target.value })}
            className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 resize-none" rows="3" />
        </div>
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" disabled={saving} className="flex-1 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-xl hover:from-blue-700 hover:to-blue-800 transition shadow-lg disabled:opacity-60">
    ...
    <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl" onMouseDown={e => e.stopPropagation()}>
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6 rounded-t-2xl flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="p-2 bg-white/20 rounded-lg"><Receipt className="h-6 w-6" /></div>
          <div>
      ...
      <div className="p-6 space-y-4">
        <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 rounded-xl">
          <div><p className="text-xs text-slate-500">Date</p><p className="font-semibold text-slate-800">{formatDate(expense.expense_date)}</p></div>
          <div><p className="text-xs text-slate-500">Time</p><p className="font-semibold text-slate-800">{formatTime(expense.expense_time) || "—"}</p></div>
          <div>
          ...
          <div><p className="text-xs text-slate-500">Amount</p><p className="text-xl font-bold text-blue-600">{formatCurrency(expense.amount)}</p></div>
        </div>
        <div className="p-4 bg-slate-50 rounded-xl">
          <p className="text-xs text-slate-500 mb-1">Note</p>
          <p className="text-sm text-slate-700">{expense.note || <i className="text-slate-400">No note provided</i>}</p>
        </div>
        <div className="flex justify-end gap-3 pt-2 border-t border-slate-100">
          ...
          <button onClick={onEdit} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition flex items-center gap-2">
...
// ─── Category Add/Edit Modal ──────────────────────────────────────────────────
const CategoryModal = ({ category, formData, setFormData, saving, error, onClose, onSubmit }) => (
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4"
    onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
    <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl" onMouseDown={e => e.stopPropagation()}>
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-4 rounded-t-2xl flex items-center justify-between">
        <div>
          ...
          <p className="text-purple-100 text-xs mt-1">{category ? "Update details" : "Create a new category"}</p>
        </div>
        <button onClick={onClose} className="p-1 hover:bg-white/20 rounded-lg"><X size={20} /></button>
      </div>
      <form onSubmit={onSubmit} className="p-5 space-y-4">
        ...
        <div>
          <label className="block text-gray-700 font-medium text-sm mb-2">Name <span className="text-purple-600">*</span></label>
          <input type="text" placeholder="e.g. Marketing, Travel, Office…" value={formData.name}
        ...
        <div>
          ...
          <textarea placeholder="Brief description (optional)" value={formData.description}
            onChange={e => setFormData({ ...formData, description: e.target.value })}
            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 resize-none text-sm" rows="2" />
        </div>
        <div className="flex gap-3 pt-1">
          <button type="button" onClick={onClose} className="flex-1 py-2 border border-slate-300 text-gray-700 font-semibold text-sm rounded-lg hover:bg-slate-50 transition">Cancel</button>
