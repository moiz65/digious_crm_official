...
app.use(
  `/api/${process.env.API_VERSION}/checkout-missing`,
  checkoutMissingRoutes,
);
...
app.use(`/api/${process.env.API_VERSION}/invoices`, invoiceRoutes);
...
app.use((err, req, res, next) => {
  ...
  if (err && (err.type === "entity.too.large" || err.status === 413)) {
    return res.status(413).json({
      success: false,
      message:
        "Payload too large. The server limits request body size — upload smaller files or use multipart upload.",
      ...
      if (absentEmployees.length > 0) {
        for (const emp of absentEmployees) {
          try {
            ...
            console.error(
              `   ⚠️ Error marking employee ${emp.id} as absent:`,
              err.message,
            );
          }
        }
...
async function removeAbsenceWhenAttendanceExists() {
  ...
  try {
    ...
    if (conflicting.length > 0) {
      console.log(
        `   Found ${conflicting.length} conflicting records to remove`,
      );

      for (const record of conflicting) {
        try {
          await connection.query(`DELETE FROM Employee_Absent WHERE id = ?`, [
            record.id,
          ]);
          console.log(
            `   ✅ Removed: ${record.name} (${record.email}) from ${record.absent_date}`,
          );
        } catch (err) {
          console.error(
            `   ⚠️ Error removing record ID ${record.id}:`,
            err.message,
          );
        }
      }
      ...
      console.log(
        `   ✅ No conflicting records found - all absence records are valid`,
      );
    }
  ...
  const dailySchedule = schedule.scheduleJob(dailyAbsentRule, async () => {
    ...
    try {
      ...
      if (absentEmployees.length > 0) {
        ...
        console.log(
          `✅ SCHEDULED JOB COMPLETE: ${createdCount} absence records created for ${today}`,
        );
      } else {
        console.log(
          `✅ SCHEDULED JOB COMPLETE: All employees checked in for ${today}`,
        );
      }
